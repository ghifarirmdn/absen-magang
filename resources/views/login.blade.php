...
<body class="bg-gray-50">
    <div class="flex min-h-full mt-40 flex-col justify-center my-20 sm:mt-52 sm:flex sm:items-center md:flex md:items-center lg:flex lg:items-center xl:flex xl:items-center 2xl:flex 2xl:items-center">
        ...
        <div class="mt-10 bg-white rounded-xl p-5 sm:w-1/2 md:w-1/2 lg:w-1/3 xl:w-1/3 2xl:w-1/3">
            <form class="space-y-6" action="{{ route('action_login') }}" method="POST">
                @csrf
                ...
                <div class="flex justify-center">
                    <button type="submit" class="flex w-full justify-center rounded-md bg-orange-400 px-3 py-1.5 text-sm font-semibold leading-6 text-white shadow-sm hover:bg-orange-500 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 md:w-1/2 md:h-10 md:items-center lg:h-12 lg:text-lg 2xl:mt-10 2xl:w-1/2">Sign in</button>
                </div>
            </form>

            <p class="mt-10 text-center text-sm text-gray-500 md:text-base">
            Gapunya Akun?
                <a href="/register" class="font-semibold leading-6 text-orange-400 hover:text-orange-500">Monggo Daftar</a>
            </p>
        </div>
    </div>
</body>
